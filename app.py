...
from db import get_latest_reading, get_history, get_statistics, insert_readings
...
async def api_upload(file: UploadFile = File(...)):
    """Uploads a LibreView CSV export to backfill historical glucose data."""
    ...
    try:
        # Parse CSV file
        # Default to local America/Los_Angeles timezone, but could be extended
        readings = parse_libreview_csv(tmp_path)
        if not readings:
            return {"message": "No valid glucose readings found in the uploaded file.", "inserted": 0}
